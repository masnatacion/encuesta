...
class GoogleGeocode extends BaseGeocode
{
	...
	public function geocode( $address )
	{
		...
		$url .= urlencode( $address ) . "&hl=es&output=xml&oe=UTF-8&key=" . $this->apiKey;
		...
		if( $file['response'] == 200 ) {
			...
			if( $xml && $retVal['Response']['Status'] == GoogleGeocode::SUCCESS )
			{
				...
				$countries = $xml->xpath('//urn:CountryName');
				$countriesCodes = $xml->xpath('//urn:CountryNameCode');
				...
				for( $i = 0; $i < count( $placemarks ); $i++ )
				{
					...
					$retVal['Placemarks'][$i]['CountryCode'] = (string)$countriesCodes[$i];
